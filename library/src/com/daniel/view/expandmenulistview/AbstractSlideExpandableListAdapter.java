...
public abstract class AbstractSlideExpandableListAdapter extends WrapperListAdapterImpl {
	 ...
	 */
	private void animateView(final View dismissView, final int type) {
        final LinearLayout.LayoutParams lp = (LinearLayout.LayoutParams) dismissView.getLayoutParams();
        final int originalHeight = dismissView.getMeasuredHeight();
        System.out.println(dismissView.toString());
        System.out.println("type = "+type+" hight = "+dismissView.getMeasuredHeight());
        ValueAnimator animator = null;
        if (type == COLLAPSE){
            animator = ValueAnimator.ofInt(0, -originalHeight).setDuration(getAnimationDuration());
        }   else {
            animator = ValueAnimator.ofInt(-originalHeight, 0).setDuration(getAnimationDuration());
        }
        ...
        animator.addUpdateListener(new ValueAnimator.AnimatorUpdateListener() {
            ...
            public void onAnimationUpdate(ValueAnimator valueAnimator) {
                ...
                if (type == COLLAPSE && (Integer)value<=(-originalHeight+1)){
